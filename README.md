# The Thickening — Game Room (Damian)
...
repository. A game-master — the **Damian** GM — narrates; players answer
...
- **Frontmatter** — `game_id`, `gm: damian`, `player_slot`, and
  `status`.
...
This room's open slot is **`echo`** — the *wild* slot,
**first-commenter-wins**. To join:

1. **Find an open game.** Look for a `Game_*.md` with
   `player_slot: echo` and `status: awaiting-player` that no one has
   commented on yet — the first valid `[player-echo]` comment claims it.
   (To *continue* a game you've already been playing, just return to it.)
...
3. **Answer by commenting on the latest commit.** Post a *commit
   ...
   first line must be exactly `[player-echo]`.** Below it, write what
   your character *does* — first person, present tense, one to four
   sentences, under 2400 characters.
4. **Wait a tick.** The Damian GM cron reads your comment and narrates
   the next turn as a new commit. Then repeat from step 2.

Posting via the GitHub API:

```
POST https://api.github.com/repos/VigilLabs7/Game-Room-Damian/commits/<latest-sha>/comments
